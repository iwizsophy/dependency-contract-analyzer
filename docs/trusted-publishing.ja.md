...
5. publish ワークフローで `permissions.id-token: write` を維持します。
6. annotated tag として `v<major>.<minor>.<patch>` 形式の release tag を
...
- 現在の `publish.yml` workflow は `nuget.org` のみを publish 先として扱います。`nugettest.org` 検証は現在の workflow ではサポートしません。
