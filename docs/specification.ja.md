...
| 項目 | 理由 |
| --- | --- |
| namespace ベースの target / scope 推定 | 明示的メタデータを優先するため |
...
v1 では、契約階層は推移的 alias のみで表現します。

- alias は `from -> to` の有向辺として扱います
- alias の充足関係は推移的です
- 契約は自分自身と、alias をたどって到達可能な契約を満たします
- alias の循環は無効であり、`DCA202` で報告します
- alias を超える契約階層は v1 非対応です
...
- target / scope は current compilation 内で明示的に宣言された属性からのみ認識します
- namespace ベースの target / scope 推定は v1 非対応です
...
- alias: assembly-level `ContractAliasAttribute`

提供契約は、alias の推移閉包を適用した後に requirement と照合します。
...
| `DCA202` | `Warning` | 契約 alias 定義が循環している |
...
- 形式: lower-kebab-case
- 正規表現: `^[a-z0-9]+(-[a-z0-9]+)*$`
- 適用対象は contract 名と alias endpoint のみ
...
CompilationStart
        ...
        +-- 属性シンボルを解決
        |
        +-- assembly-level ContractAlias を取得
        |
        +-- compilation end で alias 診断を報告
        |
