...
`dependency_contract_analyzer.external_dependency_policy = metadata` の場合、参照先 assembly からも explicit provided contract / target / scope を dependency matching に使います。参照先 assembly は namespace inference 名を供給せず、undeclared target / scope 判定も current compilation のままです。
