...
- GitHub の default branch は `main` です。
- `feature/*`、`bugfix/*`、`chore/*` の通常の Pull Request ターゲットは
  `develop` です。
...
- release tag は annotated tag とし、
  `v<major>.<minor>.<patch>` 形式を使います。
- tag publish の publish 先は branch の意図で決まり、
  `main` のみに到達可能な commit の tag は `nuget.org`、
  `develop` のみに到達可能な commit の tag は
  `https://int.nugettest.org/` に publish します。
- tag 対象 commit が `main` と `develop` の両方から到達可能、
  またはどちらからも到達不可能な場合は publish を失敗させます。
- GitHub Release は `main` 上の release tag から作成します。
- manual publish workflow dispatch は `develop` と `main` でのみ
  実行します。
- `develop` からの manual dispatch は `https://int.nugettest.org/`
  へ publish します。
- `main` からの manual dispatch は `https://www.nuget.org/` へ
  publish します。
- required CI checks は `build`、`test`、`analyzer`、`pack` です。
